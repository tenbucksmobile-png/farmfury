...
- **World map restyled to `SunriseMeadows_New.png`.** All 18 `PathPositions` re-measured directly from the new mockup (1280×720) using an automated color-blob detector cross-checked against pixel-grid crops, then converted to canvas pixels via the mockup's exact 1.5× scale into the 1920×1080 reference resolution (both share a 16:9 aspect, and the background `Image` stretches non-aspect-preserving to fill the canvas, so this mapping is exact regardless of the sprite's imported PPU) — no longer the original spec's `unit * 100` scheme. **Pin *positions* are measured, not guessed; pin *ordering* (which pin = which level number) is a best-guess left-to-right path traversal, NOT verified against a hand-traced path** — flagged in `WorldMapController.cs` itself; tell Claude if level numbers need reordering once seen live. Confirmed the mockup's uniform metallic-icon pin art is exactly `LevelMarker_Locked.png` repeated as a placeholder (pixel-identical crop comparison) — real dynamic markers (Locked/Unlocked/1-3 star) render on top of and fully occlude the baked-in placeholder pins at the same measured coordinates, so no background touch-up was needed.
- **Position indicator now slides to the new pin before bobbing**, not snapping — `WorldMapController.RefreshMarkers()` detects when the furthest-unlocked level changed since the last refresh and reports it to a single `IndicatorRoutine()` coroutine that eases over (SmoothStep, 0.7s) before resuming the existing up/down bob. Deliberately kept to ONE coroutine ever touching `_playerIndicatorRT` — an earlier draft used two concurrent coroutines (slide + bob) that fought over the same RectTransform every frame.
- **NEXT LEVEL and Home buttons added**, per the mockup — `Btn_nextlevel.png`/`Btn_home.png` (already imported, previously unused). NEXT LEVEL is a shortcut equivalent to tapping whichever pin the bobbing indicator currently sits on; Home returns to the Main Menu (same as the old text-based back button it replaced). The old code-drawn "SUNRISE MEADOWS" TMP title was dropped — the new background art bakes the banner in directly, so the old text was doubling up.
- **`LevelPreviewCard` replaced by `MatchUpScreen`** — a full-screen "animal VS robot" transition matching the new `MatchUp_New.png` mockup (framed animal card left, framed robot card right, VS graphic between), instead of the old centred popup. **Both cards are read fresh from `GameManager.GetLevelData(levelIndex)` on every `Show()`** — animal is `birds[0]`, robot is `robots[0].robotType` — per explicit user clarification that this is not a static "chicken vs robot" splash; a different level shows a different matchup. Player-selectable animal (mentioned by the user as a planned future feature once animals are unlockable) was explicitly NOT built — `Show()` always uses the level's `birds[0]`. The mockup has no visible continue button (verified: the source image is a full, uncropped 1280×720 frame) — implemented as tap-anywhere-to-continue with a pulsing "TAP TO CONTINUE" hint (a UX default picked here, not mockup-specified) plus a dedicated ✕ close button to back out without replaying.
- **Found and fixed a real pre-existing bug: nested runtime-built components' `[SerializeField]`s never actually get wired by the batch "Wire Scene References" pass.** `MatchUpScreen` (and the `LevelPreviewCard` it replaced) is a child `GameObject` created inside `WorldMapController.BuildUI()`, which only runs when `Awake()` fires — Play-mode runtime, or immediately after a *fresh* `AddComponent` call. Simply finding an already-serialized `WorldMap` GameObject in the scene (the normal case after the first run) does **not** re-fire `Awake()`, so `GetComponentInChildren<MatchUpScreen>()` in the editor script always found nothing — confirmed empirically by grepping the saved `Game.unity` for `m_Children: []` on `WorldMap`'s Transform, even immediately after a from-scratch `AddComponent<WorldMapController>()` in the same batch pass (i.e. `AddComponent` does *not* reliably trigger a synchronous edit-time `Awake()` in this batch-mode context, unlike normal interactive-Editor behaviour). This means the old `LevelPreviewCard`'s animal-card art field was **silently never wired for its entire existence** since Round 10 — nobody caught it because Play mode's own fresh `Awake()` would have worked fine *if* the field had ever been correctly saved, which it never was. **Fixed by moving all of `MatchUpScreen`'s art (`_matchUpBackgroundSprite`, `_vsSprite`, `_animalCardSprites`, `_robotCardSprites`) onto `WorldMapController` instead** (which persists correctly — proven by every other field on it already working) and threading them into `MatchUpScreen.Init(...)` as constructor-style parameters at `BuildUI()` time. This sidesteps the whole ordering question: it no longer matters whether `BuildUI()` transiently ran during the edit-time batch pass, only that the fields are correctly saved for the *next* real `Awake()` (Play mode / the actual game).
- **Fixed the Main Menu PLAY button rendering as a plain orange square.** `SceneSetup.EnsureMainMenu()` was wiring `_playButtonSprite` from `Assets/Sprites/UI/Play.png` — a file that has never existed in this project — so it silently fell back to `MainMenuController`'s flat-color placeholder every time. Rewired to the already-imported `Assets/Sprites/UI/Icon/Btn_play.png`. Added a matching **SETTINGS button** (`Btn_settings.png`, also already imported but previously wired to nothing) that opens a small Music/SFX toggle popup reusing `AudioManager.MusicEnabled`/`SfxEnabled`/`SetMusicEnabled`/`SetSfxEnabled` — the exact same persisted state HUDController's pause menu already toggles, so the two screens can't disagree about whether audio is on.
- **Fixed corner buttons (PLAY/SETTINGS/NEXT LEVEL/Home) rendering outside the visible safe area, clipped by the phone bezel/notch.** First attempt positioned all four via a centre-anchored `RectTransform` with a large fixed pixel offset, computed by measuring their position in one specific flat 16:9 mockup and reapplying that absolute offset directly — this only holds if the actual rendered canvas is exactly that aspect ratio. Confirmed broken via a user-supplied phone-frame screenshot showing the buttons straddling/outside a yellow safe-area guide. **Fixed by switching all four to corner-anchoring** (`anchorMin`/`anchorMax` pinned to the actual corner, `pivot` centred on the button itself, small fixed inward offset e.g. `(160, 160)`) instead of centre-anchoring with a large offset — this keeps a consistent safe inset from whichever corner is actually rendered, regardless of device aspect ratio, the same reasoning behind the pre-existing `SafeArea` wrapper HUDController uses for notches. **Not yet visually re-confirmed on a real device/build** — the reasoning is sound and matches an established pattern in this codebase, but no screenshot has verified the corrected positions yet.
- **Removed the code-drawn "World 1 — Meadow Ruins" text label** from the Main Menu (bottom-right `Version` TMP object in `MainMenuController.BuildUI()`) per direct user request — it wasn't part of the intended landing page design.
- **Could not run the automated Unity batch compile check for the final round of fixes** (corner-anchoring + settings popup) — the user had the Unity Editor open interactively at the time (`unity/Temp/UnityLockfile` held by two live `Unity.exe` processes), and force-closing someone else's open Editor session was judged too risky to attempt unprompted. Verified via careful manual line-by-line review instead, including cross-checking the exact `AudioManager` static API being called against its source. The user's own open Editor should have auto-recompiled and would show any real error in its Console — worth a quick check before assuming this is fully verified.

**Round 12 — 2026-07-18/19. L01 robot free-fall + floating haybails (both root-caused, not guessed); mobile aim-drag hypersensitivity fixed; MatchUp and WorldMap "baked mockup + duplicate code overlay" bugs (same root-cause pattern as each other) found and fixed; new pin-free SunriseMeadows.png generated and all 18 path positions re-traced from real pixel data; robot/haybail hit-reaction art added.**
- **HarvesterRobot "starts right, then falls" — real bug, not perception.** `RobotEnemy` never set `Rigidbody2D.bodyType` anywhere in the class — the prefab shipped `Dynamic` with default gravity/no constraints (Unity's default), unlike every `BlockBase` block type, which `Initialise()` explicitly sets `Static`. So every robot free-fell from its LevelData spawn Y under gravity from frame one, settling lower than authored. Fixed by setting `_rb.bodyType = RigidbodyType2D.Static` in `RobotEnemy.Awake()`, matching the block convention — `OnCollisionEnter2D`'s effective-mass damage formula already special-cases a `Static` collision partner (used for blocks colliding with other blocks), so hit detection was unaffected, only the fall stopped.
- **L01 haybail pile "still too high" — a real, previously-unvalidated gap, distinct from the stack-gap fix in Round 10.** Round 10 correctly recomputed the *internal* gap between the top bale and the base row from `Haybail.png`'s real pixel content (0.7365u at scale 0.9) — that part was right. What nobody had checked: the base row's Y values were copied straight from the user's original hand-placed decorative props without ever validating them against the actual ground line (Y=-6.60) — they sat ~0.63 units above where a bale's bottom edge should touch, i.e. the *entire pile* (base **and** top bale riding on it) was floating with a visible gap underneath, which is what the user's screenshot showed. Fixed by shifting all 4 haybale Y values down by the same 0.627u in both `L01_FirstContact.asset` and `LevelDataGenerator.cs` (kept in sync) — preserves the base row's original hand-placed unevenness (an intentional "pile" look) while anchoring the lowest bale to the ground.
- **Mobile aim-drag "feel" — root cause measured, not guessed.** The loaded bird sits only ~0.055 world units from `CatapultLauncher`'s aim pivot (`PivotPos()`). Since the drag angle is `atan2` of the pointer's offset from that pivot, the first sliver of finger/mouse movement swings the angle across most of the 50° pull range — worse on touchscreens, where contact-point jitter is larger than a mouse's sub-pixel precision. Fixed with `MinAimRadius = 0.6f`: the drag angle now freezes (holds its last value, starting at `_armRestAngle`) until the pointer has moved at least that far from the pivot, giving the gesture a stable direction before it starts steering. Confirmed touch input itself was never the problem — `HandleInput()` already reads `Pointer.current` (covers Mouse + Touchscreen), not `Mouse.current`; `CLAUDE.md`'s old "Input System ONLY: `Mouse.current`..." line was stale documentation, not a real gap (see "Key Implementation Rules" — not yet corrected there, worth fixing next pass).
- **MatchUpScreen double-frame bug — confirmed by pixel-comparing a user screenshot against the actual background asset.** `_matchUpBackgroundSprite` was wired to `MatchUp_Background.png`, which turned out to be the *flat design mockup* — sky **and** both wood picture frames **and** the chicken/robot **and** the VS graphic, all baked into one image — not a plain backdrop as its own code comment claimed. The real modular card sprites (`Cluck_Chicken.png`/`Harvestor_Robot.png`, each already a complete framed portrait) were being drawn on top of that, producing two overlapping, slightly-rotated frames — visible in the screenshot as a second frame edge poking out from behind the real card. Fixed by rewiring `_matchUpBackgroundSprite` to `Background_SkyV1.png` (the same painted sky/ruins/hills art already used as the main gameplay backdrop, with nothing else baked in) — the only frame drawn now is each card's own.
- **WorldMapController had the exact same "baked mockup used as live background" bug, independently discovered — the pins and the NEXT LEVEL/Home buttons, not just the title.** `SunriseMeadows.png` baked in all 18 placeholder pins (all identical generic "locked" icons) *and* a "NEXT LEVEL." button *and* a Home/compass icon, while the code drew its own separate (much smaller, differently-positioned) versions of all three on top — Round 11 had already fixed this exact problem for the title text (dropped the code-drawn version, kept the baked banner) but never extended that fix to the pins/buttons, so they kept silently double-rendering. Confirmed by inspecting the PNG directly, matching pixel-for-pixel against what the user's screenshot showed.
  - User explicitly asked to stop iterating blind fixes in this area ("we cannot go back and forth over the same bugs") — asked before touching it further via `AskUserQuestion` (options: regenerate clean map art vs. hand-edit the PNG vs. keep guessing positions); got no response in time, so only applied the one change that didn't need new art or a design call (dropped NEXT LEVEL/Home to invisible click-zones over the baked art, mirroring the already-proven title fix) and left the 18 pins untouched rather than guess a fourth time.
  - **User then generated a clean replacement `SunriseMeadows.png` via Kling AI** (prompt drafted collaboratively — same scenery: barn, pond+duck, windmill, trees, fence, stone ruins, robot fortress, winding path, wood-frame border, "SUNRISE MEADOWS" banner — explicitly with **no** pins/buttons/other text baked in), same 1920×1080 as the canvas reference resolution (a happy 1:1 pixel-to-canvas-unit mapping, no scale-factor math needed this time, unlike the old 1280×720 mockup that needed a 1.5× conversion). This is the point where the invisible-click-zone fix for NEXT LEVEL/Home became a regression — the new art has nothing baked in for them either, so they'd render as literally invisible with no art underneath. **Reverted NEXT LEVEL/Home back to real rendered `Btn_nextlevel.png`/`Btn_home.png` sprites** (corner-anchored, matching the Main Menu safe-area fix), restoring the `_nextLevelButtonSprite`/`_homeButtonSprite` fields and their `SceneSetup.cs` wiring that had briefly been deleted.
  - **All 18 `PathPositions` re-traced from the new art's actual pixel data, not eyeballed.** Masked the tan dirt-path colour out of the new `SunriseMeadows.png`, isolated the path's largest connected pixel blob (via a plain-Python BFS flood-fill — no `scipy` available in this environment), built a graph over it and BFS'd geodesic distance from the pond/barn end to get an *ordered* centerline (this matters because the path is an S-curve that loops back on itself — a naive per-column-average centerline breaks badly at the bend), smoothed it (moving-average, window 11) to remove BFS-in-a-wide-blob noise, then walked 18 evenly arc-length-spaced points along the smoothed curve. Verified visually by rendering all 18 points back onto the source image before committing to the coordinates (three iterations — raw BFS-bin sampling, then unsmoothed Euclidean resampling, both still showed visible jumbling at the path's tightest hairpin bend, until the smoothing pass fixed it). Level 1 = pond/barn end, level 18 = fortress end — the path's own visible start/end, not a guess (resolves Round 11's flagged "pin ordering is best-guess, not verified" note). One known rough spot, not a measurement error: levels 7-11 sit close together at the path's tightest bend, which is physically only ~60-90 canvas units wide there versus each marker's own 56-unit width — some visual crowding at that one bend is close to unavoidable given the art.
- **Added hit-reaction art for HarvesterRobot and Haybail**, replacing/supplementing the old plain-colour flash feedback. `RobotEnemy.FlashDamage()`: if `_robotDamagedSprite` (`HarvesterRobot_Damaged.png`) is wired, swaps to it for 0.15s instead of the old white tint, then reverts (wired only on `HarvesterRobot.prefab` — the plain `Robot.prefab` has no dedicated art yet and keeps the old white-flash fallback). `BlockBase`: new optional `_sprDamaged` field + `PlayDamageFlash()`/`DamageFlashRoutine()`, same brief-swap-then-revert pattern, wired only on `HaybaleBlock.prefab` (`Haybail_Damaged.png`); no-op (old tint/crack behaviour unchanged) for Wood/Stone, which don't have one wired. Sizing note: `HarvesterRobot_Damaged.png` is a 500×500 canvas versus the normal art's 612×408, so it needed a computed custom PPU (2140, matching rendered height) to avoid visibly jumping size during the flash; `Haybail_Damaged.png` happens to already share `Haybail.png`'s exact 500×500 canvas, so the existing `WireBlockPrefab` auto-PPU-by-texture-width logic matches them for free.
...
2.6 Main menu — full-screen `LandingPage.png` splash + PLAY/SETTINGS icon buttons (2026-07-13, replaced the earlier orange-rect + "▶ PLAY" text; the PLAY icon was actually wired to a nonexistent `Play.png` and silently fell back to a plain square until Round 11 fixed it to `Btn_play.png` and added SETTINGS).
2.7 Sunrise Meadows world map (2026-07-15, restyled 2026-07-16) — winding-path level select for World 1: 18 tappable markers, slide-then-bob player-position indicator, tap-to-preview full-screen matchup screen. See `WorldMapController.cs`/`LevelMarker.cs`/`MatchUpScreen.cs` and Round 10/11 below for the full design/trade-off writeup.
...
unity/Assets/Scripts/
  ...
  UI/
    ...
    MainMenuController.cs   — LandingPage.png background (title + character art baked directly
                              into the art, no code-drawn text over it); PLAY (bottom-left) and
                              SETTINGS (bottom-right) icon buttons, both corner-anchored (see
                              Round 11); PLAY opens WorldMapController, not LevelSelectController;
                              SETTINGS opens a small Music/SFX toggle popup reusing
                              AudioManager.MusicEnabled/SfxEnabled — same state HUDController's
                              pause menu toggles, so the two can't disagree
    WorldMapController.cs    — Sunrise Meadows (World 1) level map; ScreenSpaceOverlay Canvas,
                              sortingOrder 300; 18 LevelMarker instances positioned via
                              PathPositions — mockup-measured canvas pixel coordinates as of
                              Round 11, not the original spec's X/Y*100 scheme (see Round 11);
                              unlock rule (level 1 always unlocked, level N needs level N-1 >=1
                              star) reuses ScoreManager.GetBestStars(), not a separate
                              PlayerPrefs scheme; position indicator slides to the newly-unlocked
                              pin then bobs there (IndicatorRoutine, Round 11); NEXT LEVEL
                              (bottom-left) and Home (bottom-right) buttons, both corner-anchored;
                              owns a MatchUpScreen instance, and holds ITS art fields too (see
                              Round 11 for why they can't live on MatchUpScreen itself)
    ...
    MatchUpScreen.cs         — full-screen "animal VS robot" transition screen (Round 11,
                              replaced the old centred LevelPreviewCard popup); animal card is
                              GameManager.GetLevelData(index).birds[0], robot card is
                              robots[0].robotType — read fresh per Show() call, so different
                              levels show different matchups; tap-anywhere-to-continue (a UX
                              default, not specified by the mockup) plus a dedicated ✕ close
                              button; shows "COMING SOON" when the level index has no LevelData
                              yet; art fields are NOT [SerializeField] on this component (see
                              Round 11) — passed into Init() by WorldMapController instead
...
- **Input System ONLY:** `using UnityEngine.InputSystem;` — `UnityEngine.Input` is incompatible. `CatapultLauncher.HandleInput()` and `AnimalBase`'s tap-to-trigger check use `Pointer.current` (not `Mouse.current`) — `Pointer` is the shared base class for `Mouse`/`Pen`/`Touchscreen`, so this covers touch input on mobile without needing the `EnhancedTouch` module (that's only needed for multi-finger tracking). `Pointer.current.press.wasPressedThisFrame`/`.isPressed`/`.wasReleasedThisFrame`, `pointer.position.ReadValue()` → `Vector2`.
